...
module.exports	= (function () {
	var _			= require('uquery'),
		...
		defaults	= {
			'columns': {
				'date_created': ['date_created', 'created', 'created_at'],
				'date_modified': ['date_modified','modified', 'changed_at']
			}
		},
	
	MySQL	= function (db, table_name, id_column, options) {
		...
		return {
			...
			'getInsertData': function (data) {
				...
				for (index = 0; index < columns.length; index += 1) {
					...
					options.columns.date_created.forEach(function (col) {
						if (column === col && !value) {
			...
			'getUpdateData': function (data) {
				var modified_cols	= options.columns.date_modified, 
					prop;
				
				for (prop in data) {
					if (data.hasOwnProperty(prop)) {
						modified_cols.forEach(function (col) {
							if (prop === col) {
								delete data[prop];
							}
						});
