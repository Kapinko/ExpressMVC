(function () {
	...
	module.exports	= function (MVC, server) {
		...
		return function (req, res, next) {
			//Check for a locale cookie
			locale		= req.cookies.locale || locale;
			...
			res.render	= function (template, options) {
				...
				if (typeof strings === 'function') {
					...
					MVC.logger.warning('Invalid i18n file (must be a function): ' + file);
				}
				
				if (!typeof layout === 'function') {
					layout	= layout(options);
				} else {
					MVC.logger.warning('Invalid i18n layout file (must be a function): ' + file);
				}
				
				options		= options || {};
				
				if (strings || layout) {
					_.merge(true, options, layout, strings);
				}
