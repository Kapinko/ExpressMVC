...
(function () {
    ...
    function md5_sign(key, base_string, encoding) {
    ...
    vows.describe('oAuth object').addBatch({
        ...
        'Adding a signature method': {
            topic: function () {
                oAuth.addSignatureMethod('HMAC-MD5', md5_sign);
                
                var oauth   = twitter_oauth();
                oauth.setSignatureMethod('HMAC-MD5');
                
                return oauth;
            },
            'should allow the user to request that method': function (topic) {
                var key         = consumer_secret + '&' + token_secret,
                    expected    = md5_sign(key, oauth_nonce, 'base64');
                    
                assert.strictEqual(topic.sign(oauth_nonce), expected);
            }
        }
