...
module.exports  = (function () {
    ...
    /**
     * An express route/server plugin that will look at the incoming request
     * for an OAuth header an automatically parse it out into the req.oauth
     * @param {HTTPRequest} req
     * @param {HTTPResponse} res
     * @param {function()} next
     */
    OAuthHeader.request	= function (req, res, next) {
		if (!req.oauth) {
			var authorization = req.headers.Authorization || 
					req.headers.authorization;
		
			if (authorization) {
				req.oauth = OAuthHeader.parse(authorization);
			}
		}
		next();
    };
